...
// conversion state  & district  Objects to response Objects

const convertStateObjToResponseObject = (dbObject) => {
  return {
...
function authenticationToken(request, response, next) {
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, "DDUGKY", async (error, payload) => {
      if (error) {
        response.status(401);
        response.send("Invalid JWT Token");
      } else {
...
app.post("/login/", async (request, response) => {
  ...
  if (dbUser === undefined) {
    ...
    if (isPasswordMatched === true) {
      ...
      const jwtToken = jwt.sign(payload, "DDUGKY");
...
app.get("/states/", authenticationToken, async (request, response) => {
  ...
  response.send(
    statesArray.map((eachState) => convertStateObjToResponseObject(eachState))
  );
});
...
WHERE
 state_id = ${stateId};`;
  ...
  response.send(convertStateObjToResponseObject(state));
});
